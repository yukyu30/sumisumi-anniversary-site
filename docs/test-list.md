...
- [ ] 四隅がファインダー
- [ ] タイミング行・列の交互性
- [ ] 全セルが排他的に分類される（64×18 全数チェック）

### ⑦ encode
- [ ] 行列サイズ 64×18
- [ ] layout 経由のビット往復（encode → データセル読み出し）
- [ ] 容量超過で例外

### ⑧ rasterize
- [ ] scale=20 で 1280×360 の RGBA
- [ ] モジュール色が 0 or 255 の二値
- [ ] 任意 scale 対応

## Phase 3: デコード（頑健性）

- [ ] ⑨ 無劣化ラスタからのラウンドトリップ（scale 20）
- [ ] ⑩ 整数縮小（1/2, 1/4 平均プーリング）後のデコード
- [ ] ⑪ 非整数スケール（×0.53 バイリニア）後のデコード
- [ ] ⑫ 輝度シフト（+40/−40）・コントラスト低下後のデコード
- [ ] ⑬ box blur（3×3 ×2 回）後のデコード
- [ ] ⑭ jpeg-js q=70 / q=50+50% 縮小（← Reed-Solomon 要否の判断ゲート）
- [ ] ⑮ 乱数ノイズ画像 → 型付きエラー（ゴミを返さない）
- [ ] ⑯ 180° 回転画像 → 検出してエラー
